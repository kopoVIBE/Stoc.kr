...
export default function InvestmentSurvey({ isOpen, onClose }) {
  const [step, setStep] = useState(0)
  const [answers, setAnswers] = useState({})
  const [showResult, setShowResult] = useState(false)
  const [resultType, setResultType] = useState(investmentTypes[0])

  const handleCheckboxChange = (checked, optionText) => {
    const currentAnswers = answers[step] || []
    const newAnswers = checked ? [...currentAnswers, optionText] : currentAnswers.filter((ans) => ans !== optionText)
  ...
  const calculateResult = () => {
    ...
    if (totalScore <= 10)
      setResultType(investmentTypes[0]) // 안정형
    else if (totalScore <= 18)
      setResultType(investmentTypes[1]) // 안정추구형
    else if (totalScore <= 26)
      setResultType(investmentTypes[2]) // 위험중립형
    else if (totalScore <= 34)
      setResultType(investmentTypes[3]) // 적극투자형
      ...
      <DialogContent className="sm:max-w-lg">
        {!showResult ? (
          <>
            <DialogHeader>
              <DialogTitle>
                <span className="text-primary font-bold">{step + 1}</span>/{surveyQuestions.length}
              </DialogTitle>
              <p className="text-lg font-semibold pt-2">{currentQuestion.question}</p>
            </DialogHeader>
            <div className="py-4">
              {currentQuestion.type === "radio" ? (
                <RadioGroup onValueChange={(value) => setAnswers({ ...answers, [step]: value })} value={answers[step]}>
                  {currentQuestion.options.map((option) => (
                    <div key={option.text} className="flex items-center space-x-2 py-2">
                  ...
                  {currentQuestion.options.map((option) => (
                    <div key={option.text} className="flex items-center space-x-2 py-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => handleCheckboxChange(checked, option.text)}
                      />
                      <Label htmlFor={option.text} className="text-base">
                        {option.text}
                      </Label>
          ...
          <>
            <DialogHeader>
              ...
              <p className="text-center text-xs text-gray-500 pt-2">2027년 6월 27일까지 유효해요.</p>
            </DialogHeader>
            ...
            <div className="my-4">
              ...
              <div className="relative w-full h-4">
                {investmentTypes.map((type, i) => (
                  <div
                    key={type.name}
                    className={`absolute top-0 text-xs text-center w-1/5 ${type.name === resultType.name ? "font-bold" : ""}`}
                    style={{ left: `${i * 20}%` }}
                  >
                    {type.name.slice(0, 2)}
                    {type.name === resultType.name && <div className="w-1 h-1 bg-black rounded-full mx-auto mt-1" />}
                  </div>
                ))}
              </div>
            </div>
