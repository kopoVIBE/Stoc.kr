...
def read_stocks_from_csv(filename='crawler/stocks.csv'):
...
async def fetch_article_details(page, url, news_type, stock_info=None):
    ...
    try:
        ...
        if not source_element:
            print(f"    - ❌ 출처를 찾을 수 없습니다: {url}")
            return None
        source = source_element.get('alt', '').strip()
        
        category_elements = soup_article.select('em.media_end_categorize_item')
        category = [el.text.strip() for el in category_elements] if category_elements else ['미분류']

        datetime_element = soup_article.select_one('.media_end_head_info_datestamp_time')
        published_at_str = datetime_element.get('data-date-time') if datetime_element else None
        
        # published_at_str이 None이거나 빈 문자열인 경우 처리
        if not published_at_str:
            print(f"    - ❌ 발행일시를 찾을 수 없습니다: {url}")
            return None

        # 썸네일 수집 로직 (og:image)
        thumbnail_element = soup_article.select_one('meta[property="og:image"]')
        thumbnail_url = thumbnail_element.get('content') if thumbnail_element else None
...
async def main():
    ...
    print(f"'{CRAWL_MODE}' 모드로 EC2 뉴스 크롤링을 시작합니다...")
