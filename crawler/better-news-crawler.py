...
def read_stocks_from_csv(filename='stocks.csv'):
    ...
    try:
        ...
        logger.info(f"'{filename}'에서 {len(stocks)}개 종목 정보를 읽었습니다.")
        return stocks
...
async def main():
    ...
    if final_crawled_data:
        ...
        try:
            ...
            logger.info(f"기사 수집이 성공적으로 종료되어 progress.json 파일을 삭제합>니다.")
        except FileNotFoundError:
            logger.error(f"progress.json 파일을 찾을 수 없습니다.")
        except PermissionError:
            logger.error(f"progress.json 파일 삭제 권한이 없습니다.")
        except Exception as e:
            logger.error(f"progress.json 파일 삭제 중 오류가 발생했습니다: {e}")
